<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
		...
		<tr class="inputblock">
			...
			<div>
				<textarea>
					...
					<xsl:attribute name="rows"><xsl:value-of select="@size div 10 +1"/></xsl:attribute>
	...
	<xsl:template name="option">
		...
		<xsl:for-each select="options/option">
			<option>
				<xsl:attribute name="text"><xsl:value-of select="$selval"/></xsl:attribute>
				<xsl:attribute name="value"><xsl:value-of select="@value"/></xsl:attribute>
	...
	<xsl:template name="file">
		<tr class="inputblock">
			...
			<td>
				...
				<xsl:if test="value != ''">
					<span>Текущий фай: <xsl:value-of select="value"/></span>
					<span class="del"><a href="/files/del-{@id}/rd-1/">удалить</a></span>
				</xsl:if>
				<input>
	...
	<xsl:template name="filefi">
		<tr class="inputblock">
			...
			<td>
				...
				<xsl:if test="value != ''">
					<span>Текущий файл: <xsl:value-of select="value"/></span>
					<!-- <span><a href="http://{//page/@host}/files/del/"</span> -->
				</xsl:if>
				<input>
					<xsl:attribute name="type">hidden</xsl:attribute>
					<xsl:attribute name="name">inputName_<xsl:value-of select="@name"/></xsl:attribute>
			...
			<xsl:if test="isDescr = 1">
				<tr class="inputblock">
					<td>Описание:</td>
					<td>
					<xsl:variable name="fd"><xsl:text>filedescription_</xsl:text><xsl:value-of select="@name"/></xsl:variable>
						<textarea>						
							<xsl:attribute name="cols"><xsl:value-of select="@size"/></xsl:attribute>
							<xsl:attribute name="rows"><xsl:value-of select="@size div 10 +1"/></xsl:attribute>
							<xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute>
							<xsl:attribute name="{$fd}"><xsl:value-of select="@name"/></xsl:attribute>
							<xsl:attribute name="class"><xsl:value-of select="@className"/></xsl:attribute>
							<xsl:attribute name="type"><xsl:value-of select="@type"/></xsl:attribute>
