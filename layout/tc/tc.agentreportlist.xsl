...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match="container[@module = 'travellist']">
		<xsl:if test="//page/@isAjax != 1">
			<div id="cpanel">
				<table class="adminform" width="100%">
					<tbody>
						<tr>
							<td valign="top">
								<h2>Отчет по бронированиям Агентов</h2>
								...
								<div style="float: right;">
									<div class="btn-group">
										...
										<input class="btn btn-default btn-sm" type="button" onclick="buttonSetFilter('langFilter', '1', 'ajax','input', 'http://{//page/@host}/{//page/@name}/viewAgentReportList-1/xls-1/', true)" value=" Excel "/>
									</div>
								</div> 
							</td>
						</tr>
			...
			<form id="langFilter" name="langFilter" method="post" action="" style="display:none;">
				<table>
					<tbody>
						<tr>
							<td>
					...
					<thead>
						...
						<xsl:for-each select="travel/item">
							<tr>
								...
								<td><xsl:if test="new_site = 1"><xsl:attribute name="class">new_site</xsl:attribute></xsl:if>
									<xsl:value-of select="(count(../../travel/item)-position()+1)"/></td>
								<td><xsl:value-of select="agent_name"/></td>
								<td><xsl:value-of select="tab_no"/></td>
								<td><xsl:value-of select="tur_name"/></td>
								<td><xsl:value-of select="tur_date"/></td>
								<!-- <td>
