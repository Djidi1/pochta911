...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    <xsl:template match="container[@module = 'carslist']">
        ...
        <hr/>
        <div id="printlist">
            <table class="table table-hover table-stripsed table-condensed datatables">
                <thead>
                    <tr>
                        <th>№ [ID]</th>
                        <th>ФИО</th>
                            ...
                            <xsl:if test="count(//page/@xls)=0">
                                ...
                                <td width="40px" align="center">
                                    <a href="/admin/userBan-{id}/" title="удалить" class="btn btn-danger btn-xs">
                                        <xsl:attribute name="onClick">return confirm('Вы действительно хотите удалить <xsl:value-of select="name"/>?');
                                        </xsl:attribute>
