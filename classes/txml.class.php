...
/*
  protected function addNode ($nodeName, $nodeValue) {
    #stop($nodeName.' '.$nodeValue,0);
    #$node = $this->xml->createElement($nodeName, $nodeValue);
    #stop($this->xml->saveXML(),0);
    #stop($node->tagName,0);
...
*/
 ...
 protected function addToNode ($nodeName, $newNode, $nodeValue = '') {
    ...
    if (gettype($nodeValue) != 'object'){    	
    	...
    	else $item = $this->xml->createElement($newNode);
...
protected function addToNodeCDATA ($nodeName, $newNode, $nodeValue, $htmlspecialchars = false) {
    ...
    if (gettype($nodeValue) != 'object'){    	
    	if($nodeValue) {
    		if ($htmlspecialchars) $preData = htmlspecialchars($nodeValue);
    		$preData = $nodeValue;
    		//$preData = str_replace(array('&lt;![CDATA[', ']]&gt;','[ &lt;br/&gt; ]'), array('<![CDATA[',']]>','<br />'), $preData);
    		// $preData = str_replace(array('&lt;![CDATA[', ']]&gt;','&lt;br/&gt;'), array('<![CDATA[',']]>','<br />'), $preData);
    		// $item = $this->xml->createElement($newNode, $preData);
...
*/
  ...
  */
   ...
   * @param $CDATA - Массив названий полей, которые должны быть преобразованы в CDATA
  	...
  	foreach ($array as $key => $val) {
  		...
  		switch (gettype($val)) {
  			case 'array':
  				...
  				$CN = $this->arrToXML($val,$subItem, 'array',$CDATA, $recursion);
  				$this->addAttr('count',count($val),$CN);
  				$insert = false;
  				break;
  			case 'boolean': if($val) $val = 'true'; else $val = 'false'; break;
  			case 'string': $val = strval($val);  break;
  			case 'integer': $val = intval($val); if ($val == 0) $val = '0'; break;
  			case 'float': 
  			case 'double':	$val = floatval($val); if ($val == 0) $val = '0'; break;
  			case 'object': 
 ...
 */
  ...
  public function title($title) {
    $t = $this->addToNode($this->header, 'title', $title);
  }
...
if (DEBUG == 1) fwrite($fp,  'Root XSL page: '.$pRoot.rn);
    ...
    if (DEBUG == 1) { fwrite($fp, rn.join(', ',$pXSL).rn);
     fclose($fp);
